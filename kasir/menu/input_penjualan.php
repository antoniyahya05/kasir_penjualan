        ...
        <div class="col-md-4">
            <?php
                if (isset($_POST['proses'])) {
                    ...
                    $profil_i = $profil['id_kasir'];
                    $total_i = $total['total'];

                    $tanggal = date('Y-m-d');
                    $query1 = mysqli_query($koneksi, "insert into tb_penjualan(id_buku, jumlah, jumlah_harga, id_jual) select id_buku, jumlah,jumlah_harga, '$kode_otomatis' from tb_keranjang");
                    // masukkan data ke tb_jual
                    $query = mysqli_query($koneksi, "insert into 'tb_jual'('id_jual', 'total','uang','kembali','id_kasir','tanggal') values ('$kode_otomatis', '$total_i', '$uang', '$kembali', '$profil_i', '$tanggal')");
